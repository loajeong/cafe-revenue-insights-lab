...
menu_db = {
    ...
    'HerbTea':   {'price': 5000, 'cost_hot': 470, 'cost_ice': 500, 'category': 'Tea'},   # [Fixed] HubTea -> HerbTea
    'Ade':       {'price': 6000, 'cost_hot': 3000, 'cost_ice': 3000, 'category': 'Beverage'}, 
    'Financier': {'price': 2900, 'cost_hot': 1000, 'cost_ice': 1000, 'category': 'Dessert'} 
}
...
while current_date <= end_date:
    ...
    if month in [12, 1, 2]: # Winter
        daily_orders = int(np.random.normal(50, 5)) 
        season = 'Winter'
    elif month in [3, 4, 5, 6, 7, 8]: # Spring/Summer
        daily_orders = int(np.random.normal(80, 8)) 
        season = 'Summer' 
    else: # Autumn (Sep, Oct, Nov)
        daily_orders = int(np.random.normal(75, 5)) 
        season = 'Autumn'
        
    # --- [Rule 2] Random Weather Generation ---
    weather = 'Sunny'
    if season == 'Summer' and random.random() < 0.3: 
        weather = 'Rain'
    elif season == 'Winter' and random.random() < 0.1: 
        weather = 'Snow'
    ...
    for _ in range(daily_orders):
        ...
        # --- [Rule 3] Menu Category Selection ---
        ...
        # Specific Menu Selection & Temperature Decision
        menu_name = ''
        is_ice = True # Default
        ...
        if category_choice == 'Coffee':
            ...
            menu_name = 'HerbTea'  # [Fixed] HubTea -> HerbTea
...
print("File saved successfully! Please check 'cafe_sales_data_en.csv'.")
